...
int main(int argc, char** argv)
{
    ...
    Vec y_err{1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8,1.e-8};
    Vec p0{700,1,1};
    Vec lb{1e-6,1e-6,1e-6};
    Vec ub{1e3,1e2,1e2,1e2,1e2};
    double tol = 1.e-10;
    test t;
    int args0 = y.size();
    std::tuple<int> args{args0};
    Vec p(p0), p_err(p0);
    std::cout << "Finished with result: " << least_squares::fit(y,y_err,p0,p,p_err,lb,ub,tol,t.calc_Pls_from_Pl_exp_ps,t.calc_grad_Pls_from_Pl_exp_ps,args,false) << std::endl;
    for(unsigned i=0; i<p.size(); i++){
        std::cout << y[i] << " " << p[i] << " " << p_err[i] << std::endl;
    }
    auto Pls = t.calc_Pls_from_Pl_exp_ps(p,33);
    for(unsigned l=0; l<Pls.size(); l++)
        std::cout << l << " " << Pls[l] << " " << y[l] << std::endl;

    p0.resize(2); p0[0] = 1; p0[1] = 1;
    std::cout << "Finished with result: " << least_squares::fit(y,y_err,p0,p,p_err,lb,ub,tol,t.calc_Pls_from_Pl_inv_ps,t.calc_grad_Pls_from_Pl_exp_ps,args,false) << std::endl;
    for(unsigned i=0; i<p.size(); i++){
        std::cout << y[i] << " " << p[i] << " " << p_err[i] << std::endl;
    }
    Pls = t.calc_Pls_from_Pl_inv_ps(p,33);
    for(unsigned l=0; l<Pls.size(); l++)
        std::cout << l << " " << Pls[l] << " " << y[l] << std::endl;
